...
export interface TelegramAgentScheduledTask {
  id: number;
  created_at: string;
  updated_at: string;
  name: string;
...
export interface TelegramAgentScheduledTaskPayload {
  name: string;
  prompt: string;
  enabled: boolean;
  schedule_type: TelegramAgentScheduleType;
...
export async function getTelegramAgentScheduledTasks(): Promise<TelegramAgentScheduledTask[]> {
  return apiRequest<TelegramAgentScheduledTask[]>("/tg-agent/scheduled-tasks");
}

export async function createTelegramAgentScheduledTask(payload: TelegramAgentScheduledTaskPayload): Promise<TelegramAgentScheduledTask> {
  return apiRequest<TelegramAgentScheduledTask>("/tg-agent/scheduled-tasks", {
    method: "POST",
    body: JSON.stringify(payload),
  });
...
export async function updateTelegramAgentScheduledTask(id: number, payload: TelegramAgentScheduledTaskPayload): Promise<TelegramAgentScheduledTask> {
  return apiRequest<TelegramAgentScheduledTask>(`/tg-agent/scheduled-tasks/${id}`, {
    method: "PUT",
    body: JSON.stringify(payload),
  });
}

export async function updateTelegramAgentScheduledTaskStatus(id: number, enabled: boolean): Promise<TelegramAgentScheduledTask> {
  return apiRequest<TelegramAgentScheduledTask>(`/tg-agent/scheduled-tasks/${id}/status`, {
    method: "PATCH",
    body: JSON.stringify({ enabled }),
  });
}

export async function deleteTelegramAgentScheduledTask(id: number): Promise<void> {
  await apiRequest<void>(`/tg-agent/scheduled-tasks/${id}`, {
    method: "DELETE",
  });
}
