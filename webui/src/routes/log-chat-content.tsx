...
import type { ChatIO } from "@/lib/api";
...
const LazyJsonContent = lazy(() => import("./log-chat-json-content"));
const LazyMarkdownOutputText = lazy(() => import("./log-chat-markdown"));

function JsonContent(props: JsonContentProps) {
  return (
    <Suspense fallback={<div className="rounded-md border bg-muted/40 p-4 text-sm text-muted-foreground">正在准备内容...</div>}>
      <LazyJsonContent {...props} />
    </Suspense>
  );
}
...
function MarkdownOutputText({ value }: { value: string }) {
  return (
    <Suspense fallback={<div className="text-sm text-muted-foreground">正在渲染 Markdown...</div>}>
      <LazyMarkdownOutputText value={value} />
    </Suspense>
  );
}
...
function StructuredInputCard({ raw }: StructuredInputCardProps) {
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        {viewMode === "raw" || !parsedAvailable || payload === null ? (
          <JsonContent text={formatted.text} parsed={formatted.parsed} empty={formatted.empty} />
        ) : (
          <>
...
function StructuredOutputCard({ raw }: { raw: string }) {
  const [viewMode, setViewMode] = useState<ViewMode>("parsed");
  const formatted = useMemo(() => formatJson(raw), [raw]);
...
export default function LogChatContent({ chatIO }: { chatIO: ChatIO }) {
  ...
  return (
    <div className="space-y-6">
      <StructuredInputCard raw={chatIO.Input} />
      <StructuredOutputCard raw={normalizedOutputRaw} />
    </div>
  );
}
